    ...
    <div class="container">
      <div class="row">
        <div class="col-sm-3 fondbodyl">
          ...
          <p style="color:white;"><strong><?=$_SESSION['firstname']?>
          </strong><br>
            <em style="color:#8899a6">@<?=$_SESSION['username']?></em></p>
          <div class="row">
        ...
        <div class="col-lg-9">
          <div class="twit fondhead form-tweet">
            <div class="center">
              <form class="" action="index.php?c=update" method="post">
                <h2>Username</h2>
                <p>@<?=$_SESSION['username']?></p>
                <div class='form-group'><input type='text'
                  class='form-control form-controlH' id='username'
                  aria-describedby='username' placeholder='Enter new username'
                  name='username'></div>
                <h2>Email</h2>
                <p><?=$_SESSION['email']?></p>
                <div class='form-group'><input type='email' class='form-control
                  form-controlH' id='exampleInputEmail1'
                  placeholder='Enter new Email' name='email'></div>
                <h2>Password</h2>
                <div class='form-group'><input type='password'
                  class='form-control form-controlH' id='password'
                  placeholder='Current Password' name='curent_password'></div>
                <div class='form-group'><input type='password'
                  class='form-control form-controlH' id='password_confirm'
        ...
        <div class="twit fondhead form-tweet col-lg-6">
          <div class="center">
            ...
            <?php $cnt1->execute();
            while ($id = $cnt1->fetch()) {
              $followers = $job->followers($id);
              $followers->execute();
              $follower = $followers->fetch();
              ?>
              <a href="index.php?c=Extprofile&p=<?=$follower['username']?>"><p><img src="<?= $follower['avatar'] ?>"
                class="profil" alt="profil">
                @<?= $follower['username'] ?></p></a>
              <?php
            } ?>
          </div>
        </div>
        <div class="twit fondhead form-tweet col-lg-6">
          <div class="center">
            ...
            <?php $cnt2->execute();
            while ($id = $cnt2->fetch()) {
              $following = $job->following($id);
              $following->execute();
              $followin = $following->fetch();
              ?>
              <a href="index.php?c=Extprofile&p=<?=$followin['username']?>"><p><img src="<?= $followin['avatar'] ?>"
                class="profil" alt="profil">
                @<?= $followin['username'] ?></p></a>
              <?php
            } ?>
          </div>
        </div>
